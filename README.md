# 👋 Hola, soy Oscar Cáceres

🎯 Python Developer & Data Science Trainee  
💳 +10 años de experiencia en banca – Tarjetas de Crédito (QA, Dev, Análisis Funcional)  

---

## 📌 Sobre mí

Soy desarrollador Python con formación en Data Science y una sólida experiencia en el sector bancario, especialmente en **tarjetas de crédito**, donde trabajé en roles de **QA, desarrollo y análisis funcional**.

Me enfoco en escribir **código claro, modular y mantenible**, aplicando buenas prácticas, testing y análisis de datos para resolver problemas reales.

Actualmente continúo fortaleciendo mis habilidades en **Python, análisis de datos y visualización**, con el objetivo de integrarme a equipos técnicos orientados a datos y backend.

---

## 🛠️ Tecnologías y herramientas

* **Lenguajes:** Python, SQL, JavaScript, Java  
* **Data Science:** Pandas, Matplotlib, Jupyter  
* **Backend & Dev:** Git, GitHub, Pytest  
* **Otros:** HTML, CSS

---

## 📂 Portfolio de proyectos

👉 **Repositorio principal de proyectos:**  
🔗 [https://github.com/csodcaceres/Proyectos_Python](https://github.com/csodcaceres/Proyectos_Python)

Cada proyecto incluye README propio, estructura organizada y foco en legibilidad y aprendizaje progresivo.

---

## 📈 Objetivo profesional

Continuar creciendo como **Python Developer orientado a Data Science**, aportando valor en proyectos de análisis, automatización y desarrollo backend.

---

## 📫 Contacto

* 📧 Email: **[byttonica@gmail.com](mailto:byttonica@gmail.com)**  
* 💼 [LinkedIn – Oscar Daniel Cáceres](https://www.linkedin.com/in/oscardanielcaceres/)  
* 🐙 GitHub: [https://github.com/csodcaceres](https://github.com/csodcaceres)

---

⭐ *Gracias por visitar mi perfil. Siempre abierto a aprender, mejorar y colaborar en nuevos desafíos.*

---

## ⚡ GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=csodcaceres&show_icons=true&theme=default)  
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=csodcaceres&layout=compact)
